...
def main():
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            page.wait_for_function("document.querySelector('#auto-rows')?.textContent==='19'",timeout=30000)
            assert page.locator('#auto-rows-body tr').count()==19
            ...
            assert pages and max(pages)<=71 and all(p in pages for p in (23,24,25,58,59,60))
            ...
            assert len(auto['rows'])==19 and all(max(r['source_pages'])<=71 for r in auto['rows'])
            ...
            report['checks'].append('Public HTTPS Automatic BOQ rendered nineteen scored rows with 95% audit-subset coverage; FCO/CO/RFD/AVC are drawing-tag counts and floor drain is intentionally withheld at four explicit tags')
            report['checks'].append('Public generated JSON proves reference isolation and all published evidence pages <= 71')
