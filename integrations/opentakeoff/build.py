...
def main() -> None:
    ...
    upstream=args.upstream.resolve(); web=upstream/'web'; out=args.output.resolve()
    ...
    for path in (web/'src').rglob('*'):
        ...
        old=path.read_text()
        new=re.sub(r'(["\'`])/demo/',r'\1./demo/',old)
    ...
    # Local-only POC. No provider/API credentials, cloud syncing or external uploads.
    ...
    env={**os.environ,'VITE_ONE_CLICK':'0','VITE_COMMAND_BOX':'0','VITE_CLOUD_SYNC':'0',
         'VITE_GOOGLE_CLIENT_ID':'','VITE_DRIVE_ROOT_FOLDER_ID':''}
    subprocess.run(['npm','run','build','--','--base=./'],cwd=web,env=env,check=True)
    out.mkdir(parents=True,exist_ok=True)
    for name in ('index.html','poc.css','poc.js','poc-report.mjs'):
    ...
    (engine/'POC-NOTICE.txt').write_text('Modified downstream by Blender3d: local-only entry point; isolated demo/user storage; synthetic metric demo; read-only BOQ adapter; relative demo URLs. Measurement/totals engine from Kentucky-ai/opentakeoff, Apache-2.0.\n')
    # Do not advertise the upstream project's MCP service as ours.
    ...
    (out/'build-info.json').write_text(json.dumps({**PIN,'built_at':datetime.now(timezone.utc).isoformat(),
        'source_commit':os.environ.get('GITHUB_SHA','local'),'upstream_tests':'passed','adapter_tests':'passed',
        'limitations':['no IFC reconciliation','no AI or MCP endpoint','no cloud persistence','preliminary quantities only']},indent=2))
