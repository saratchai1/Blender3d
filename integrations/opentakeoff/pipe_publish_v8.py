...
def publish_validated_pipe_rows(result: dict[str, Any], release: dict[str, Any]) -> dict[str, Any]:
    ...
    if (
        release.get('exact_segment_evidence_required') is True
        and release.get('exact_segment_evidence_status') != 'PASS_EXACT_SOURCE_GEOMETRY_RECONCILED'
    ):
    ...
    out.setdefault('diagnostics', []).append({
        ...
        'release_gate_status': release.get('status'),
        'exact_segment_evidence_required': release.get('exact_segment_evidence_required') is True,
        'exact_segment_evidence_status': release.get('exact_segment_evidence_status'),
