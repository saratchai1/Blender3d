...
def detect_hatched_roof(page: fitz.Page, scale_ratio: float, slope_deg: float = 0.0) -> dict[str, Any]:
    ...
    for seg in horizontals:
        buckets.setdefault((round(seg[0] / 3), round(seg[2] / 3)), []).append(seg)
    ...
    mpp = m_per_point(scale_ratio); projected = width_pt * height_pt * mpp * mpp; slope_factor = 1.0 / math.cos(math.radians(slope_deg)) if slope_deg else 1.0
    return {"status":"DETECTED","bounds_pt":[round(x0,3),round(y0,3),round(x1,3),round(y1,3)],"width_m":width_pt*mpp,"height_m":height_pt*mpp,"projected_area_m2":projected,"surface_area_m2":projected*slope_factor,"perimeter_m":2*(width_pt+height_pt)*mpp,"slope_deg":slope_deg,"hatch_lines":len(dense),"confidence":min(0.98,0.80+min(len(dense),40)/250)}
...
def detect_swing_doors(page: fitz.Page, scale_ratio: float, mapped_widths_m: dict[str, str]) -> list[dict[str, Any]]:
    ...
    for drawing in page.get_drawings():
        if len([it for it in drawing.get("items", []) if it[0] == "c"]) != 1:
            continue
        r=drawing["rect"]; w,h=r.width,r.height
...
def tight_dark_crop(arr: np.ndarray, threshold: int = 200, pad: int = 2) -> np.ndarray:
    ...
    return arr[max(0,int(ys.min())-pad):min(arr.shape[0],int(ys.max())+pad+1),max(0,int(xs.min())-pad):min(arr.shape[1],int(xs.max())+pad+1)]


def template_sweep(template_page: fitz.Page, template_rect_pt: list[float], target_pages: list[tuple[int, fitz.Page]], target_roi_pt: list[float], threshold: float, render_scale: float = 1.2, rotate_90: bool = False) -> list[dict[str, Any]]:
    ref=render_gray(template_page,render_scale); x0,y0,x1,y1=[int(round(v*render_scale)) for v in template_rect_pt]; tpl=tight_dark_crop(ref[y0:y1,x0:x1]); templates=[(0,tpl)]
    if rotate_90:
        templates.append((90,cv2.rotate(tpl,cv2.ROTATE_90_CLOCKWISE)))
    all_hits=[]
    for page_no,page in target_pages:
        arr=render_gray(page,render_scale); rx0,ry0,rx1,ry1=[int(round(v*render_scale)) for v in target_roi_pt]; crop=255-arr[ry0:ry1,rx0:rx1]; raw=[]
        ...
        for h in merged:
            all_hits.append({"page":page_no,"x_norm":h["x_px"]/arr.shape[1],"y_norm":h["y_px"]/arr.shape[0],"match_score":h["score"],"rotation":h["rotation"]})
    return all_hits
...
def detect_text_label_counts(page: fitz.Page, specs: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    words=page.get_text("words") or []; out=[]
    for spec in specs:
        tokens={str(t).strip().upper() for t in spec.get("tokens",[spec.get("token","")]) if str(t).strip()}; bounds=spec.get("bounds_pt"); hits=[]
        for w in words:
...
def quantity_row(item_id: str, description: str, category: str, unit: str, quantity: float, confidence: float, method: str, pages: list[int], evidence: dict[str, Any], review: str = "REVIEW_REQUIRED") -> dict[str, Any]:
    return {"id":item_id,"description":description,"category":category,"unit":unit,"quantity":round(float(quantity),3),"confidence":round(float(confidence),3),"method":method,"source_pages":sorted(set(pages)),"review":review,"evidence":evidence}
...
def extract(pdf_path: Path, profile_path: Path) -> dict[str, Any]:
    ...
    guarded=GuardedPdf(doc,max_page); rows=[]; diagnostics=[]

    roof_cfg=profile.get("roof")
    if roof_cfg:
        p=int(roof_cfg["page"]); rr=detect_hatched_roof(guarded.page(p),float(roof_cfg["scale_ratio"]),float(roof_cfg.get("slope_deg",0))); diagnostics.append({"detector":"roof_hatch_band","page":p,**rr})
    ...
    if door_cfg:
        ...
        for p in door_cfg["pages"]:
            ds=detect_swing_doors(guarded.page(int(p)),float(door_cfg["scale_ratio"]),door_cfg["type_by_nominal_width_m"])
    ...
    if elec:
        legend_p=int(elec["legend_page"]); legend=guarded.page(legend_p); targets=[(int(p),guarded.page(int(p))) for p in elec["lighting_pages"]]
        for detector in elec.get("symbol_templates",[]):
            hits=template_sweep(legend,detector["template_rect_pt"],targets,detector["target_roi_pt"],float(detector["threshold"]),float(elec.get("render_scale",1.2)),bool(detector.get("rotate_90",False))); median=float(np.median([h["match_score"] for h in hits])) if hits else 0.0
            if hits:
                rows.append(quantity_row(detector["id"],detector["description"],"Electrical","ea",len(hits),median,"raster:legend-template sweep",[legend_p]+[h["page"] for h in hits],{"legend_page":legend_p,"detections":hits,"threshold":detector["threshold"],"template":detector.get("template_name",detector["id"])}))
            diagnostics.append({"detector":f"electrical_template:{detector['id']}","legend_page":legend_p,"target_pages":elec["lighting_pages"],"detections":len(hits),"median_score":median,"status":"DETECTED" if hits else "WITHHELD"})

    for r in rows:
        if any(p>max_page for p in r["source_pages"]):
            raise AssertionError(f"reference leakage: {r['id']} cites a source page after {max_page}")
    coverage={
        "supported_detectors":["roof/fascia vector hatch band","0.70/0.80 m swing-door counts","sanitary exact plan-label counts","electrical legend-template sweeps"],
        "withheld_detectors":[
            {"name":"windows","status":"WITHHELD","reason":"type/count mapping not yet reliable on this CAD-font set"},
            {"name":"sanitary fixtures","status":"WITHHELD","reason":"fixtures beyond validated equipment tags still need a plan-view fingerprint library"},
...
def main() -> None:
    ap=argparse.ArgumentParser(); ap.add_argument("--pdf",type=Path,required=True); ap.add_argument("--profile",type=Path,required=True); ap.add_argument("--output",type=Path,required=True); args=ap.parse_args(); result=extract(args.pdf,args.profile); args.output.parent.mkdir(parents=True,exist_ok=True); args.output.write_text(json.dumps(result,ensure_ascii=False,indent=2),encoding="utf-8"); print("AUTO_BOQ_OK",json.dumps({"rows":len(result["rows"]),"ids":[r["id"] for r in result["rows"]],"output":str(args.output)},ensure_ascii=False))
