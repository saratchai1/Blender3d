...
def main():
    ...
    required={'ARCH-ROOF-METAL','ARCH-FASCIA','ARCH-DOOR-D2','ARCH-DOOR-D3','SAN-BOOSTER-PUMP','SAN-WATER-METER','SAN-FLOAT-VALVE','SAN-WC-BOWL','SAN-LAVATORY','SAN-SHOWER-SET','ELEC-DOWNLIGHT','ELEC-T8-LONG'}
    ...
    assert by['SAN-LAVATORY']['quantity']==3
    assert by['SAN-LAVATORY']['source_pages']==[23,24,25]
    assert by['SAN-LAVATORY']['confidence']>=0.80
    assert by['SAN-SHOWER-SET']['quantity']==2
    assert by['SAN-SHOWER-SET']['source_pages']==[24,25]
    assert by['SAN-SHOWER-SET']['confidence']>=0.75
    ...
    assert result['reference_rows']==13
    assert result['detected_reference_rows']==12
    assert result['coverage_pct']>=92.3
    assert result['detected_rows_accuracy_pct']==100
    assert result['mean_absolute_error_pct']<0.25
